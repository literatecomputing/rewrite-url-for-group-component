...
RSpec.describe "Rewrite URLs for group members", system: true do
  ...
  fab!(:post) do
    Fabricate(
      ...
      topic: topic,
      raw:
        "[old](https://dronescene.co.uk/page) [new](https://beta.dronescene.co.uk/page)",
    )
  end
  ...
  it "rewrites old_url links to new_url for group members" do
    ...
    expect(page).to have_css(
      "a[href='https://beta.dronescene.co.uk/page']",
      count: 2,
    )
  end
  ...
  it "rewrites new_url links back to old_url for non-members when rewrite_all_urls is on" do
    ...
    visit(topic.url)
    expect(page).to have_css(
      "a[href='https://dronescene.co.uk/page']",
      count: 2,
    )
  end
